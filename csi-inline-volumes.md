...
* VolumeHandle in in-line volumes can have prefix. This prefix (Pod UID, Namespace UID or nothing) is added to the VolumeHandle before each CSI call. It makes sure that each pod uses a different volume ID for its ephemeral volumes. 
	* Each Pod created by ReplicaSet, StatefulSet, DaemonSet will get the same copy of a Pod template. `CSIVolumeHandlePrefixPod` makes sure that each pod gets its own unique volume ID and thus can get 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTM4MTc1NjI3MSwtMTQ2MTY1MTMzMywtMT
